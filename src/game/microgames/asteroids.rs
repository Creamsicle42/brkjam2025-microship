...
impl Default for Data {
    fn default() -> Self {
        Data {
            asteroid_pos: Vec2::new(gen_range(150.0, 850.0), gen_range(150.0, 450.0)),
...
pub fn update(data: &mut Data, input: FrameInput, delta: f32) -> bool {
    ...
    if data.lazer_pos > 900.0 {
        data.lazer_pos = 900.0;
        data.lazer_vel *= -1.0;
    }
    ...
    if input.mouse_state == MousePressState::JustPressed && !data.did_shoot {
        ...
        if (data.lazer_pos - data.asteroid_pos.x).abs() < 60.0 {
            data.did_hit = true;
        }
    }
...
pub fn draw(data: &Data) {
    ...
    if !data.did_shoot {
        draw_rectangle(data.lazer_pos - 5.0, 0.0, 10.0, 700.0, PINK);
    }
    if !data.did_hit {
        draw_rectangle(
